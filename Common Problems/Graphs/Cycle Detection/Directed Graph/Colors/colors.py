...
def has_cycle_directed(graph):
    ...
    distances = {node:WHITE for node in graph.adjacent_list}
    
    def dfs(node):
        distances[node] = GREY
        for neighbour in graph.adjacent_list[node]:
            if distances[neighbour] == GREY:
                return True
            if distances[neighbour] == WHITE and dfs(neighbour):
                return True
        distances[node] = BLACK
        return False
    
    for vertex in graph.adjacent_list:
        if distances[vertex] == WHITE and dfs(vertex):
            return True
    return False
