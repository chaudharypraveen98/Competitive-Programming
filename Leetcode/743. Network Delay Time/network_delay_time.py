...
class Solution:
    def networkDelayTime(self, times: List[List[int]], n: int, k: int) -> int:
        adjacent_list = {source:{} for source in range(1, n+1)}
        for source, target, weight in times:
            adjacent_list[source][target] = weight
        def djiskstra(node):
            distances = {target:float('inf') for target in range(1,n+1)}
            distances[node]  = 0
            heap = [(0,node)]
            max_time_needed = 0
            ...
            visited = set()
            while heap:
                cur_weight, target = heapq.heappop(heap)
                if cur_weight > distances[target]:
                    continue
                visited.add(target)
                max_time_needed = max(max_time_needed, cur_weight)
                total_nodes_visited +=1
                distances[target] = cur_weight
                for neighbour  in adjacent_list[target]:
                    n_weight = adjacent_list[target][neighbour]
                    if cur_weight + n_weight > distances[neighbour]:
                        continue
                    if neighbour not in visited:
                        heapq.heappush(heap,(n_weight+cur_weight, neighbour))
            is_any_unvisited = any(distances[distance]==float('inf') for distance in distances)
            return -1 if is_any_unvisited else max_time_needed
        return djiskstra(k)
