    ...
    def findTheCity(self, n: int, edges: List[List[int]], distanceThreshold: int) -> int:
        ...
        def dijkstra(src):
            ...
            while heap:
                ...
                if cost > distances[item]:
                    continue
                
                for  val, neigbhour in graph[item]:
                    new_cost = val+cost
                    if new_cost <= distanceThreshold and new_cost < distances[neigbhour]:
                        distances[neigbhour] = val+cost
                        heapq.heappush(heap, (val+cost, neigbhour))
            total_cities = sum(1 for i in range(n) if i!=src and distances[i] <= distanceThreshold)
            return total_cities
        ...
        for i in range(0,n):
            count = dijkstra(i)
            # Tie-breaker: '<=' ensures greater city index wins on a tie
            if count <= min_cities:
                min_cities = count
                result_city = i
        return result_city

    ## We can find answer using bfs without using visited but it will take much time and if we use visited then optimized path will be missed
    def findTheCity1(self, n: int, edges: List[List[int]], distanceThreshold: int) -> int:
        graph = {i:[] for i in range(n)}
        for u,v,w in edges:
            graph[u].append((w,v))
            graph[v].append((w,u))
        min_cities, min_item = float('inf'), -1
        def bfs(node, count, min_node):
            queue = deque([(0, node)])
            total_items = 0
            visited = set()
            while queue:
                cost,item = queue.popleft()
                visited.add(item)
