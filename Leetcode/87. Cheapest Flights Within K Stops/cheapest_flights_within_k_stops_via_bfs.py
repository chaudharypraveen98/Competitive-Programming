...
class Solution:
    def findCheapestPrice(self, n: int, flights: List[List[int]], src: int, dst: int, k: int) -> int:
        ...
        # min_cost[city] stores the cheapest cost discovered so far
        min_cost = [float('inf')] * n
        min_cost[src] = 0

        queue = deque([(0, 0,src)])
        while queue:
            current_stop, cost,item =  queue.popleft()
            if item == dst or current_stop > k:
                continue

            for neigbhour, val in graph[item].items():
                if cost+val < min_cost[neigbhour]:
                    min_cost[neigbhour] = cost+val
                    queue.append((current_stop+1, cost+val, neigbhour))
